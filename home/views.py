...
def dynamic(request):
    ...
    if request.method == "POST":
        ...
        for every_val_in_c1 in country1.values():
            ...
            Scraping_Valuesc1 = soupc1.findAll(class_=['TextBlackBold8'])
            Scraping_Country = soupc1.find("select", {"name": "cboTeams"}).findAll("option")
            
            print("-------------------------------------------")
            print(html_code_c1)
            print("-------------------------------------------")

            print("Scraping values -------------", Scraping_Valuesc1)
            print("Scraping Country -------------", Scraping_Country)
            ...
            print("this is the printed----",Country)
            s= str(Country[0])
            start = s.find("(")
            end = s.find(")")
        ...
        for every_val_in_c2 in country2.values():
            ...
            Scraping_Valuesc2 = soupc2.findAll(class_=['TextBlackBold8'])
            Scraping_Country_for_c2 = soupc2.find("select", {"name": "cboTeams"}).findAll("option")
